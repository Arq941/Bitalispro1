...
export default function QuickClientIntake(){
 ...
 return <AppShell title="Alta rápida"><main className="mx-auto max-w-xl px-3 py-4">
  ...
  <form onSubmit={submit} className="mt-3 rounded-[24px] border border-[var(--bitalis-border)] bg-white p-3 sm:p-4">
   ...
   <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3"><div className="flex items-center gap-2 text-[11px] font-black text-[var(--bitalis-primary)]"><Images className="h-4 w-4"/>FOTOS · CÁMARA O GALERÍA</div><p className="mt-1 text-[10px] text-slate-500">Puedes tomar una foto nueva o elegir una imagen existente del teléfono.</p><div className="mt-3 grid grid-cols-3 gap-2"><PhotoCapture title="Fachada" icon={<Home/>} file={photos.facade} onPick={file=>pick('facade',file)}/><PhotoCapture title="Cliente" icon={<UserRound/>} file={photos.clientPhoto} onPick={file=>pick('clientPhoto',file)}/><PhotoCapture title="Contrato" icon={<FileCheck2/>} file={photos.contract} onPick={file=>pick('contract',file)}/></div></div>
   <button type="button" onClick={extractContract} disabled={!photos.contract||ocrLoading||saving} className="mt-3 flex min-h-16 w-full items-center justify-center gap-2 rounded-2xl border-2 border-violet-300 bg-violet-50 px-4 text-sm font-black text-violet-800 shadow-sm disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-400 disabled:opacity-100">{ocrLoading?<Loader2 className="h-5 w-5 animate-spin"/>:<Sparkles className="h-5 w-5"/>}{ocrLoading?'EXTRAYENDO CONTRATO…':photos.contract?'EXTRAER OCR DEL CONTRATO':'AGREGA EL CONTRATO PARA EXTRAER OCR'}</button>
