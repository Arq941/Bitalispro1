...
export default function QuickClientIntake(){
 ...
 return <AppShell title="Alta rápida"><main className="mx-auto w-full max-w-xl px-3 py-4 sm:px-4">
  <section className="rounded-[24px] bg-[var(--bitalis-primary)] p-4 text-white shadow-[0_10px_30px_rgba(6,43,36,.12)]"><p className="text-[10px] font-black uppercase tracking-[.14em] text-[var(--bitalis-mint)]">CRM en campo</p><h1 className="mt-1 text-xl font-black">Alta rápida</h1><p className="mt-1 text-xs leading-5 text-emerald-50/80">Captura los datos principales, evidencia y ubicación. Gemini puede ayudarte a revisar la calidad antes de guardar.</p></section>
  ...
  <form onSubmit={submit} className="bitalis-form-card mt-3 p-3 sm:p-4">
   <div className="mb-4 flex flex-wrap items-start justify-between gap-2 border-b border-slate-100 pb-3"><div className="min-w-0 flex-1"><p className="text-sm font-black text-[var(--bitalis-primary)]">Datos del cliente</p><p className="mt-0.5 text-[11px] leading-4 text-slate-500">Los campos con * son necesarios. La revisión de Gemini no modifica tus datos.</p></div><span className="shrink-0 rounded-full bg-emerald-50 px-2.5 py-1 text-[9px] font-black text-[var(--bitalis-action-dark)]">{completed}/4 EVIDENCIAS</span></div>
   <label className="block"><span className="bitalis-form-label">Nombre del cliente *</span><input ref={nameRef} autoComplete="name" enterKeyHint="next" value={name} onChange={e=>{setName(e.target.value);invalidateReview();}} placeholder="Nombre completo" className="min-h-14 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-base outline-none focus:border-[var(--bitalis-action)] focus:ring-4 focus:ring-emerald-500/10"/></label>
   <label className="mt-3 block"><span className="bitalis-form-label">Teléfono</span><input type="tel" inputMode="tel" autoComplete="tel" enterKeyHint="next" value={phone} onChange={e=>{setPhone(e.target.value);invalidateReview();}} placeholder="Número telefónico" className="min-h-14 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-base outline-none focus:border-[var(--bitalis-action)] focus:ring-4 focus:ring-emerald-500/10"/><span className="bitalis-form-help">Usa el número principal donde pueda localizarse al cliente.</span></label>
   <label className="mt-3 block"><span className="bitalis-form-label">Notas de la visita <span className="font-medium text-slate-400">(opcional)</span></span><textarea value={notes} onChange={e=>{setNotes(e.target.value.slice(0,800));invalidateReview();}} placeholder="Referencia, detalle importante o pendiente que deba revisar el supervisor" className="min-h-24 w-full resize-y rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-base outline-none focus:border-[var(--bitalis-action)] focus:ring-4 focus:ring-emerald-500/10"/><span className="bitalis-form-help text-right">{notes.length}/800</span></label>
   ...
   <section className="bitalis-form-section mt-3"><div className="flex items-center gap-2 text-[11px] font-black text-[var(--bitalis-primary)]"><Images className="h-4 w-4 shrink-0"/>EVIDENCIAS · CÁMARA O GALERÍA</div><p className="mt-1 text-[10px] leading-4 text-slate-500">Adjunta fachada, cliente y contrato. Gemini no recibe ni analiza estas fotografías.</p><div className="mt-3 grid grid-cols-1 gap-2 min-[360px]:grid-cols-3"><PhotoCapture title="Fachada" icon={<Home/>} file={photos.facade} onPick={file=>pick('facade',file)}/><PhotoCapture title="Cliente" icon={<UserRound/>} file={photos.clientPhoto} onPick={file=>pick('clientPhoto',file)}/><PhotoCapture title="Contrato" icon={<FileCheck2/>} file={photos.contract} onPick={file=>pick('contract',file)}/></div></section>

   <section className="mt-3 rounded-2xl border border-violet-200 bg-violet-50/70 p-3"><div className="flex items-start gap-2"><Sparkles className="mt-0.5 h-5 w-5 shrink-0 text-violet-700"/><div className="min-w-0"><p className="text-xs font-black text-violet-900">Revisión inteligente</p><p className="mt-0.5 text-[10px] leading-4 text-violet-700/80">Gemini revisa consistencia de los datos capturados y te indica qué falta antes de continuar.</p></div></div><button type="button" onClick={reviewWithGemini} disabled={aiLoading||saving||!canCreate} className="mt-3 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl border border-violet-300 bg-white px-4 text-sm font-black text-violet-800 shadow-sm disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-400">{aiLoading?<Loader2 className="h-5 w-5 shrink-0 animate-spin"/>:<Sparkles className="h-5 w-5 shrink-0"/>}{aiLoading?'REVISANDO CON GEMINI…':'REVISAR ALTA CON GEMINI'}</button></section>

   {aiReview&&<section className={`mt-3 rounded-2xl border p-3 ${aiReview.status==='READY'?'border-emerald-200 bg-emerald-50':'border-amber-200 bg-amber-50'}`}><div className="flex items-start gap-2"><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${aiReview.status==='READY'?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700'}`}>{aiReview.status==='READY'?<CheckCircle2 className="h-5 w-5"/>:<CircleAlert className="h-5 w-5"/>}</span><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[.12em]">Revisión Gemini · {aiReview.status==='READY'?'LISTA':'REVISAR'}</p><p className="mt-0.5 text-xs leading-5 text-slate-700">{aiReview.summary}</p></div></div><div className="mt-3 space-y-2">{aiReview.checks?.map((check,index)=><div key={`${check.label}-${index}`} className="flex gap-2 rounded-xl bg-white/70 p-2 text-xs"><span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${check.ok?'bg-emerald-500':'bg-amber-500'}`}/><div className="min-w-0"><b>{check.label}</b><p className="mt-0.5 leading-5 text-slate-600">{check.message}</p></div></div>)}</div><p className="mt-3 text-xs font-black text-[var(--bitalis-primary)]">Siguiente acción: <span className="font-bold text-slate-700">{aiReview.nextAction}</span></p></section>}
