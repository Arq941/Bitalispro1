...
async function recoverDelivery(){
 ...
 location.replace(`/?__bitalis_recover=${Date.now()}`);
}
...
export default function GlobalError({error,reset}:{error:Error&{digest?:string};reset:()=>void}){
 ...
 const recover=async()=>{setWorking(true);try{reset();}catch{}await recoverDelivery();};
 ...
 return <html lang="es"><body style={{margin:0}}><main style={page}><section style={card}><div style={icon}>!</div><div style={{marginTop:'16px',fontSize:'11px',fontWeight:900,letterSpacing:'.12em',color:'#0C7A46'}}>RECUPERACIÓN BITALIS</div><h1 style={{margin:'8px 0 0',fontSize:'22px',color:'#062B24'}}>BITALIS necesita recargarse</h1><p style={{margin:'10px 0 0',fontSize:'14px',lineHeight:1.55,color:'#64748B'}}>La aplicación detectó un problema de carga. Tus datos, sesión y permisos se conservarán.</p><p style={{margin:'12px 0 0',fontSize:'11px',fontWeight:800,color:'#64748B'}}>Referencia: {error?.digest||error?.name||'CLIENT'}</p><button disabled={working} type="button" style={primary} onClick={recover}>{working?'RECARGANDO…':'RECARGAR BITALIS'}</button><button disabled={working} type="button" style={secondary} onClick={()=>location.replace('/')}>VOLVER AL ACCESO</button></section></main></body></html>;
}
