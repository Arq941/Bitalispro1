...
const roleModules:Record<string,any[]>={
 ...
 ADMIN:[
  ['Centro de control','KPIs empresariales','/control-center',TrendingUp],
  ['Usuarios y permisos','Roles y accesos','/settings/users',UserCog],
  ['Clientes','CRM 360','/clients',Users],
  ['Ventas','Operación comercial','/sales',ShoppingBag],
  ['Inventario','Stock y Kardex','/inventory',Boxes],
  ['Caja','Sesiones y diferencias','/cash',Banknote],
...
export default function Dashboard(){
 ...
 const heroTitle=isCollector?'Tu jornada de cobranza':isSeller?'Captura rápida y venta en campo':isSupervisor?'Supervisión operativa en campo':'Operación empresarial';
 ...
 return <AppShell title="Inicio"><div className="mx-auto max-w-7xl px-3 py-4 sm:px-4 sm:py-5">
  <section className="overflow-hidden rounded-[28px] bg-[#12224A] p-4 text-white shadow-lg shadow-slate-900/10 sm:p-7">
   <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#C79A3B]">{role}</p><h1 className="mt-2 truncate text-2xl font-black sm:text-3xl">Hola, {name}</h1><p className="mt-2 text-sm leading-5 text-slate-300">{heroTitle}</p></div><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/10"><Sparkles className="h-5 w-5 text-[#C79A3B]"/></div></div>
   ...
   {isAdmin&&<div className="mt-5 grid grid-cols-2 gap-2"><button onClick={()=>go('/control-center')} className="min-h-14 touch-manipulation rounded-2xl bg-[#FF6A00] px-2 text-xs font-black active:scale-95 sm:text-sm">CENTRO CONTROL</button><button onClick={()=>go('/settings/users')} className="min-h-14 touch-manipulation rounded-2xl bg-white/10 px-2 text-xs font-black active:scale-95 sm:text-sm">USUARIOS</button></div>}
  </section>
  ...
  {loading?<div className="flex justify-center p-14"><Loader2 className="h-7 w-7 animate-spin text-[#12224A]"/></div>:<>
   <section className="mt-4 grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4">{kpis.map(([Icon,l,v]:any)=><K key={l} I={Icon} l={l} v={v}/>)}</section>
   <div className="mt-6 flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#C79A3B]">Accesos</p><h2 className="text-lg font-black text-[#12224A] sm:text-xl">Tu operación</h2></div>{(isAdmin||isSeller||isSupervisor)&&<button onClick={()=>go('/clients/new')} className="flex min-h-11 shrink-0 touch-manipulation items-center gap-2 rounded-xl bg-white px-3 text-xs font-black text-[#12224A] shadow-sm active:scale-95"><UserPlus className="h-4 w-4"/><span className="hidden min-[370px]:inline">Cliente</span></button>}</div>
   <section className="mt-3 grid gap-2 sm:grid-cols-2 sm:gap-3 lg:grid-cols-3 xl:grid-cols-4">{modules.map(([title,sub,path,Icon])=><button key={path} onClick={()=>go(path)} className="group flex min-h-[92px] touch-manipulation items-center gap-3 rounded-3xl border border-slate-200 bg-white p-3 text-left shadow-sm transition active:scale-[.98] sm:min-h-28 sm:p-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#12224A] text-white sm:h-12 sm:w-12"><Icon className="h-5 w-5"/></div><div className="min-w-0 flex-1"><p className="truncate font-black text-[#12224A]">{title}</p><p className="mt-1 line-clamp-2 text-xs leading-4 text-slate-500">{sub}</p></div><ChevronRight className="h-4 w-4 shrink-0 text-slate-300"/></button>)}</section>
  </>}
 </div></AppShell>;
}

function K({I,l,v}:{I:any;l:string;v:any}){return <div className="min-w-0 rounded-3xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4"><I className="h-5 w-5 text-[#C79A3B]"/><p className="mt-3 truncate text-[9px] font-black uppercase tracking-[.09em] text-slate-400 sm:text-[10px]">{l}</p><p className="mt-1 break-words text-lg font-black leading-tight text-[#12224A] sm:text-xl">{v}</p></div>}
