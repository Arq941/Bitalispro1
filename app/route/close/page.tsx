...
export default function CloseRoutePage() {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100">
    ...
    <main className="mx-auto max-w-5xl px-4 py-6">
      ...
      {!session && !loading ? <section className="mt-5 rounded-[26px] border border-white/5 bg-slate-900/70 p-8 text-center"><WalletCards className="mx-auto h-9 w-9 text-slate-600"/><h2 className="mt-3 text-lg font-black">No hay caja abierta</h2><p className="mt-2 text-sm text-slate-500">Inicia una caja desde el modo cobrador antes de cerrar jornada.</p><button onClick={() => router.push('/route')} className="mt-5 min-h-12 rounded-xl bg-emerald-500 px-5 py-3 text-xs font-black text-slate-950 active:scale-95">Volver a ruta</button></section> : session && <>
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          <article className="rounded-[26px] border border-white/5 bg-slate-900/70 p-5">
            ...
            <button disabled={saving || closed} onClick={saveCount} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3.5 text-sm font-black text-slate-950 active:scale-[.99] disabled:opacity-40">{saving?<Loader2 className="h-4 w-4 animate-spin"/>:<Banknote className="h-4 w-4"/>}Guardar arqueo</button>
          </article>
          ...
          <article className="rounded-[26px] border border-white/5 bg-slate-900/70 p-5">
            ...
            <div className="mt-3 flex min-h-12 items-center gap-2 rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs text-slate-400"><MapPin className="h-4 w-4 text-emerald-400"/>{position?'GPS listo para cierre':'Esperando ubicación GPS'}</div>
            ...
            <button disabled={saving || closed || !position || (!countResult && session.status!=='COUNTING')} onClick={closeSession} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-white px-4 py-3.5 text-sm font-black text-slate-950 active:scale-[.99] disabled:opacity-40">{saving?<Loader2 className="h-4 w-4 animate-spin"/>:<ClipboardCheck className="h-4 w-4"/>}{closed?'Jornada procesada':'Cerrar jornada'}</button>
            {closed && <button onClick={() => router.push('/dashboard')} className="mt-2 min-h-12 w-full rounded-xl border border-slate-700 px-4 py-3 text-xs font-black text-slate-200 active:scale-[.99]">Volver al dashboard</button>}
          </article>
        </section>
      </>}
    </main>
