...
import {nativeNotificationPermission,requestNativeNotificationPermission,showNewNativeNotifications} from '@/lib/notifications-client';
...
export default function NotificationsPage(){
 ...
 const[items,setItems]=useState<Notification[]>([]),[loading,setLoading]=useState(true),[working,setWorking]=useState(''),[error,setError]=useState(''),[query,setQuery]=useState(''),[filter,setFilter]=useState<Filter>('ALL'),[nativePermission,setNativePermission]=useState<string>('unsupported');
 ...
 const enableNative=async()=>{if(working)return;setWorking('permission');setError('');try{const permission=await requestNativeNotificationPermission();setNativePermission(permission);if(permission==='granted'){await showNewNativeNotifications(items.slice(0,1));haptic('success');}else if(permission==='denied')setError('Los avisos están bloqueados en Android. Actívalos desde la configuración del navegador o de la app.');else if(permission==='unsupported')setError('Este navegador no admite avisos nativos. Las alertas seguirán visibles dentro de BITALIS.');}catch{setError('No pudimos activar los avisos nativos.');}finally{setWorking('');}};
