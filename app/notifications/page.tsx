...
import {AlertTriangle,Bell,Boxes,CheckCheck,CheckCircle2,ChevronRight,Info,Loader2,MapPinned,RefreshCw,Search,ShieldAlert,Stamp} from 'lucide-react';
...
export default function NotificationsPage(){
 const[items,setItems]=useState<Notification[]>([]),[loading,setLoading]=useState(true),[working,setWorking]=useState(''),[error,setError]=useState(''),[query,setQuery]=useState(''),[filter,setFilter]=useState<Filter>('ALL');
 const load=async()=>{setLoading(true);setError('');try{const j:any=await apiClient('/api/notifications');setItems(Array.isArray(j?.data)?j.data:[]);}catch(e:any){setError(e?.message||'No pudimos cargar tus alertas.');setItems([]);}finally{setLoading(false);}};
 useEffect(()=>{void load();},[]);
 ...
 const markRead=async(item:Notification)=>{if(String(item.status).toUpperCase()==='READ'||working)return;haptic('tap');setWorking(item.id);try{await apiClient(`/api/notifications/${encodeURIComponent(item.id)}/read`,{method:'POST'});setItems(current=>current.map(x=>x.id===item.id?{...x,status:'READ',readAt:new Date().toISOString()}:x));notifyUnreadChanged();}catch(e:any){setError(e?.message||'No pudimos marcar la alerta como leída.');haptic('error');}finally{setWorking('');}};
 const markAll=async()=>{if(!counts.unread||working)return;setWorking('all');try{await apiClient('/api/notifications/read-all',{method:'POST'});const now=new Date().toISOString();setItems(current=>current.map(x=>({...x,status:'READ',readAt:x.readAt||now})));notifyUnreadChanged();haptic('success');}catch(e:any){setError(e?.message||'No pudimos marcar todas como leídas.');}finally{setWorking('');}};
 ...
 return <AppShell title="Alertas"><div className="mx-auto max-w-5xl px-3 py-3 sm:px-4 sm:py-5">
  ...
  {loading?<div className="flex justify-center p-12"><Loader2 className="h-6 w-6 animate-spin text-[var(--bitalis-primary)]"/></div>:<section className="mt-3 space-y-2">{filtered.map(item=>{const meta=priorityMeta(item.priority),Icon=meta.icon,read=String(item.status).toUpperCase()==='READ',action=actionFor(item),ActionIcon=action?.icon;return <article key={item.id} className={`rounded-2xl border p-4 shadow-sm ${read?'border-slate-200 bg-white':'border-emerald-200 bg-emerald-50/50'}`}>
   ...
   {action&&ActionIcon&&<Link href={action.href} onClick={()=>void markRead(item)} className="mt-3 flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[var(--bitalis-primary)] px-4 text-xs font-black text-white"><ActionIcon className="h-4 w-4"/>{action.label}<ChevronRight className="h-4 w-4"/></Link>}
  </article>})}{!filtered.length&&<div className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">No hay alertas que coincidan con este filtro.</div>}</section>}
 </div></AppShell>;
}
