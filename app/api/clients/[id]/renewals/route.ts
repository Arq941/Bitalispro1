  ...
  try {
    ...
    if (!hasAccess) {
      return NextResponse.json(
        { success: false, error: 'FORBIDDEN: Sin acceso a renovaciones de este cliente.' },
        { status: 403 }
      );
    }

    const prisma = PrismaService.getInstance();
    const renewals = await prisma.clientRenewal.findMany({
      where: { clientId: id },
...
export async function POST(
  ...
  try {
    ...
    const renewal = await ClientService.createRenewal(id, body, userContext);
    return NextResponse.json({ success: true, renewal }, { status: 201 });
  } catch (err: any) {
