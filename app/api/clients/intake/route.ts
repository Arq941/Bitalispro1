...
export async function POST(req:NextRequest){
 try{
  ...
  const idempotencyKey=clean(form.get('idempotencyKey'))||clean(req.headers.get('idempotency-key'));
  if(!idempotencyKey||idempotencyKey.length>200)return NextResponse.json({success:false,error:'Falta una clave idempotente válida.'},{status:400});
  const prisma=PrismaService.getInstance(),endpoint='/api/clients/intake';
  const completed=await prisma.idempotencyRecord.findUnique({where:{idempotencyKey}});
  if(completed?.endpoint===endpoint)return NextResponse.json({success:true,pendingSupervisor:true,duplicate:true},{status:200});
  if(!fullName)return NextResponse.json({success:false,error:'El nombre completo es obligatorio.'},{status:400});

  const files=new Map<string,File>();
  for(const[field]of TYPES){const value=form.get(field);if(!(value instanceof File)||value.size<=0)continue;if(!value.type.startsWith('image/'))return NextResponse.json({success:false,error:'Los archivos adjuntos deben ser fotografías.'},{status:400});if(value.size>MAX_FILE)return NextResponse.json({success:false,error:'Cada fotografía debe pesar menos de 6 MB.'},{status:400});files.set(field,value)}
  ...
  const client=await ClientService.createClient({...splitName(fullName),phone,customerType:'PENDING_SUPERVISOR',assignedSellerId:user.userId,latitude,longitude,locationAccuracy:Number.isFinite(accuracy)?accuracy:undefined,idempotencyKey},intakeUser);
  ...
  for(const[field,mediaType]of TYPES){const file=files.get(field);if(!file)continue;await ClientService.uploadMedia(client.id,{mediaType,fileContent:Buffer.from(await file.arrayBuffer()),mimeType:file.type||'image/jpeg',fileSize:file.size,latitude,longitude},intakeUser)}

  await prisma.idempotencyRecord.upsert({where:{idempotencyKey},create:{idempotencyKey,endpoint,responseBody:JSON.stringify({success:true,pendingSupervisor:true}),statusCode:201},update:{endpoint,responseBody:JSON.stringify({success:true,pendingSupervisor:true}),statusCode:201}});
