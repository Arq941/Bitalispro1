...
export async function PATCH(req:NextRequest,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  const costPrice=body.costPrice===undefined?Number(current.costPrice):Number(body.costPrice);if(!Number.isFinite(costPrice)||costPrice<0)throw new Error('Costo inválido.');
  const ints={minStock:body.minStock===undefined?current.minStock:Number(body.minStock),reorderPoint:body.reorderPoint===undefined?current.reorderPoint:Number(body.reorderPoint),maxStock:body.maxStock===undefined?current.maxStock:Number(body.maxStock)};
  if(Object.values(ints).some(v=>!Number.isInteger(v)||v<0))throw new Error('Mínimo, reorden y máximo deben ser números enteros no negativos.');if(ints.maxStock<ints.minStock)throw new Error('El stock máximo no puede ser menor al mínimo.');
  ...
  const updated=await prisma.$transaction(async tx=>{const product=await tx.product.update({where:{id},data:{sku,barcode:body.barcode===undefined?current.barcode:(String(body.barcode||'').trim()||null),name,description:body.description===undefined?current.description:(String(body.description||'').trim()||null),brand:body.brand===undefined?current.brand:(String(body.brand||'').trim()||null),categoryId:body.categoryId===undefined?current.categoryId:(String(body.categoryId||'').trim()||null),cost:costPrice,costPrice,status:status as any,...ints}});for(const p of prices){const amount=Number(p.amount);await tx.productPrice.upsert({where:{productId_priceType:{productId:id,priceType:String(p.priceType) as any}},create:{productId:id,priceType:String(p.priceType) as any,price:amount,amount,isActive:true,createdBy:ctx.userId},update:{price:amount,amount,isActive:true,updatedAt:new Date()}});}return product;});
  ...
  const product=await prisma.product.findUnique({where:{id},include:{category:true,images:true,prices:true,stocks:true}});return NextResponse.json({success:true,product});
 }catch(err:any){return NextResponse.json({success:false,error:err.message},{status:statusFromError(err,400)});}
}
...
export async function DELETE(req:NextRequest,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  if(linked>0||hasStock)return NextResponse.json({success:false,error:'Este producto ya tiene historial de ventas, inventario, pedidos o precios y no puede eliminarse. Puedes editarlo y marcarlo como DESCONTINUADO para conservar el historial.'},{status:409});
  await prisma.$transaction(async tx=>{await tx.inventoryStock.deleteMany({where:{productId:id}});await tx.productPrice.deleteMany({where:{productId:id}});await tx.productImage.deleteMany({where:{productId:id}});await tx.product.delete({where:{id}});});
  await AuditLogService.log({userId:ctx.userId,action:'PRODUCT_DELETED',entity:'Product',entityId:id,oldValues:JSON.stringify({sku:product.sku,name:product.name,status:product.status})});return NextResponse.json({success:true,id});
 }catch(err:any){return NextResponse.json({success:false,error:err.message||'No se pudo eliminar el producto.'},{status:statusFromError(err,400)});}
}
