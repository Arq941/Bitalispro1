...
export async function POST(req: NextRequest) {
  try {
    ...
    if (dbUser) {
      ...
      if(!primaryRole||!allowedRoles.includes(primaryRole as any)) return NextResponse.json({success:false,message:'La cuenta no tiene un rol operativo válido. Solicita al administrador que asigne un rol.'},{status:403});
