...
export default function SettingsPage(){
 ...
 return <AppShell title="Configuración"><div className="mx-auto max-w-5xl px-3 py-3 sm:px-4 sm:py-5"><section className="rounded-[24px] bg-[var(--bitalis-primary)] p-4 text-white sm:p-5"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.14em] text-[var(--bitalis-mint)]"><Settings className="h-4 w-4"/>Ajustes</div><h1 className="mt-2 text-xl font-black sm:text-2xl">Configuración</h1><p className="mt-1 text-sm text-emerald-50/75">Cuenta, seguridad, diagnóstico y preferencias de la aplicación.</p></section><div className="mt-3 grid grid-cols-2 gap-2 sm:mt-5 sm:gap-3">
  ...
  {isAdmin&&<button onClick={()=>router.push('/diagnostics-transition')} className="col-span-2 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-left shadow-sm sm:rounded-3xl sm:p-5"><div className="flex items-start justify-between gap-3"><div><Activity className="h-5 w-5 text-[var(--bitalis-action-dark)]"/><h2 className="mt-2 font-black text-[var(--bitalis-primary)]">Diagnóstico BITALIS</h2><p className="mt-1 text-xs leading-5 text-slate-600 sm:text-sm">Abre la traza interna, identifica el build desplegado y copia el diagnóstico cuando ocurra un error de sesión, navegación o pantalla.</p></div><span className="rounded-full bg-white px-2 py-1 text-[9px] font-black text-[var(--bitalis-action-dark)]">SOPORTE</span></div></button>}
  {isAdmin&&<button onClick={()=>window.location.assign(androidApkUrl)} className="col-span-2 rounded-2xl border border-emerald-200 bg-white p-4 text-left shadow-sm sm:rounded-3xl sm:p-5"><div className="flex items-start justify-between gap-3"><div><Download className="h-5 w-5 text-[var(--bitalis-action)]"/><h2 className="mt-2 font-black text-[var(--bitalis-primary)]">Descargar APK Android</h2><p className="mt-1 text-xs leading-5 text-slate-600 sm:text-sm">Descarga BITALIS-latest.apk. El archivo se reemplaza automáticamente después de cada compilación correcta de main.</p></div><span className="rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-black text-[var(--bitalis-action-dark)]">ANDROID</span></div></button>}
