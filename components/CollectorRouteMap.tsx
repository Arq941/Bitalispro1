...
function FitRoute({ position, stops }: Props) {
  const map = useMap();
  useEffect(() => {
  ...
  }, [map, position.lat, position.lng, stops]);
  return null;
}

export default function CollectorRouteMap({ position, stops, onSelect }: Props) {
  const line: [number, number][] = [[position.lat, position.lng], ...stops.map(s => [s.lat, s.lng] as [number, number])];
  return (
    <div className="overflow-hidden rounded-[24px] border border-white/5 bg-slate-900">
      <MapContainer center={[position.lat, position.lng]} zoom={14} scrollWheelZoom className="h-[420px] w-full">
        ...
        />
        <FitRoute position={position} stops={stops} />
        ...
        {stops.map(stop => (
          <CircleMarker
            ...
            radius={stop.active ? 11 : 8}
            pathOptions={{ weight: stop.active ? 5 : 3, fillOpacity: 0.92 }}
