...
import {ArrowLeft,Bell,Boxes,ClipboardCheck,Coins,Home,Loader2,LogOut,Menu,ReceiptText,Repeat2,Route,Settings,ShieldCheck,ShoppingCart,UserPlus,Users,WalletCards,WifiOff,X} from 'lucide-react';
...
import {haptic,installGlobalHaptics} from '@/lib/ux/haptics';
...
const supervisor:NavItem[]=[{href:'/dashboard',label:'Inicio',icon:Home,permission:'dashboard.view'},{href:'/clients',label:'Clientes',icon:Users,permission:'clients.view'},{href:'/authorizations',label:'Autorizar',icon:ShieldCheck,permission:'sales.approve'},{href:'/portfolio',label:'Cartera',icon:WalletCards,permission:'collections.view'},{href:'/control-center',label:'Control',icon:Boxes,permission:'reports.view'}];
...
function PersistentShell({children,initialTitle}:{children:ReactNode;initialTitle?:string}){
 ...
 const showBack=!fieldSeller&&pathname!=='/dashboard'&&!nav.some(item=>item.href===pathname);
 const goBack=()=>{haptic('tap');if(window.history.length>1)router.back();else router.push(getAuthenticatedLandingRoute(permissions?Array.from(permissions):[]));};

 const needed=requiredPermission(pathname);const adminOnlyDenied=role!=='ADMIN'&&isAdminExclusivePath(pathname);const denied=!publicPath&&user!==null&&permissions!==null&&(adminOnlyDenied||(fieldSeller?pathname!=='/clients/new':!!needed&&!permissions.has(needed)));
 ...
 return <ShellContext.Provider value={contextValue}><div ref={shellRef} className={`bitalis-app-shell min-h-[100svh] text-[var(--bitalis-text)] ${privateReady?'pb-24':''}`} data-shell-mode={publicPath?'public':'private'} onClickCapture={onClickCapture} onTouchStart={onSwipeStart} onTouchEnd={onSwipeEnd} onTouchCancel={onSwipeCancel}>
  <header className={privateReady?'bitalis-safe-top sticky top-0 z-40 border-b border-[var(--bitalis-border)] bg-white shadow-[0_6px_22px_rgba(6,43,36,.06)]':'hidden'}><div className="mx-auto flex min-h-[64px] max-w-7xl items-center justify-between px-2.5 py-2 sm:min-h-[68px] sm:px-4"><div className="flex min-w-0 items-center gap-2 sm:gap-3">{showBack?<button onClick={goBack} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-[var(--bitalis-border)] bg-white text-[var(--bitalis-primary)] shadow-sm" aria-label="Volver a la pantalla anterior"><ArrowLeft className="h-5 w-5"/></button>:<BitalisLogo size="md" variant="light"/>}<div className="min-w-0"><p className="text-[8px] font-black uppercase tracking-[.12em] text-[var(--bitalis-action-dark)] sm:text-[10px] sm:tracking-[.14em]">{role||'BITALIS'}</p><h1 className="max-w-[118px] truncate text-xs font-black text-[var(--bitalis-primary)] min-[360px]:max-w-[150px] min-[390px]:max-w-[180px] sm:max-w-none sm:text-sm">{shellTitle}</h1></div></div><div className="flex items-center gap-1.5">{!fieldSeller&&permissions?.has('dashboard.view')&&<button onClick={()=>go('/notifications')} className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-[var(--bitalis-border)] bg-white text-[var(--bitalis-primary)] shadow-sm" aria-label={unreadCount?`Alertas, ${unreadCount} pendientes`:'Alertas'}><Bell className="h-5 w-5"/>{unreadCount>0&&<span className="absolute -right-1 -top-1 flex min-h-5 min-w-5 items-center justify-center rounded-full bg-red-600 px-1 text-[9px] font-black leading-none text-white shadow-sm ring-2 ring-white">{unreadCount>99?'99+':unreadCount}</span>}</button>}<button onClick={()=>{haptic('tap');setAccountOpen(true);}} className="flex h-12 min-w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--bitalis-primary)] px-2.5 text-[11px] font-black text-white shadow-sm" aria-label="Cuenta y sesión">{initials}</button></div></div></header>{privateReady&&!online&&<div className="bitalis-network-banner" role="status" aria-live="polite"><WifiOff className="h-4 w-4 shrink-0"/><span>Sin conexión · algunas acciones se enviarán al recuperar internet</span></div>}
  <main className="min-h-0">{content}</main>
  <nav data-role={role} className={privateReady&&nav.length>0?'bitalis-safe-bottom fixed inset-x-0 bottom-0 z-50 border-t border-[var(--bitalis-border)] bg-white/98 px-2 pt-2 shadow-[0_-12px_30px_rgba(6,43,36,.10)] backdrop-blur-xl':'hidden'} aria-label="Navegación principal"><div className={`mx-auto grid max-w-xl gap-1 ${nav.length===1?'grid-cols-1':nav.length===2?'grid-cols-2':nav.length===3?'grid-cols-3':nav.length===4?'grid-cols-4':'grid-cols-5'}`}>{nav.map(({href,label,icon:Icon})=>{const active=href==='/dashboard'?pathname===href:pathname.startsWith(href);return <button key={href} onClick={()=>go(href)} aria-current={active?'page':undefined} className={`flex min-h-[60px] min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-1 text-[9px] font-black focus:outline-none focus:ring-2 focus:ring-[var(--bitalis-action)] sm:text-[10px] ${active?'bg-[var(--bitalis-primary)] text-white shadow-md shadow-emerald-950/10':'text-slate-500'}`}><Icon className="h-5 w-5 shrink-0"/><span className="max-w-full truncate">{label}</span></button>})}</div></nav>
