...
const admin:NavItem[]=[{href:'/dashboard',label:'Inicio',icon:Home,permission:'dashboard.view'},{href:'/control-center',label:'Control',icon:Boxes,permission:'reports.view'},{href:'/cash',label:'Caja',icon:ReceiptText,permission:'cash.view'},{href:'/inventory',label:'Stock',icon:ClipboardCheck,permission:'inventory.view'},{href:'/settings',label:'Config.',icon:Settings,permission:'settings.manage'}];
...
function PersistentShell({children,initialTitle}:{children:ReactNode;initialTitle?:string}){
 ...
 useEffect(()=>{
  ...
  const refreshPermissions=async()=>{
   ...
   try{
    const response=await fetch('/api/auth/permissions',{headers:{Authorization:`Bearer ${token}`,'Cache-Control':'no-store'},cache:'no-store'});
    if(response.status===401){sessionStorage.removeItem(permissionCacheKey);window.dispatchEvent(new Event('bitalis:session-expired'));return;}
