...
export default function GeminiCopilot(){
 ...
 return <>
  ...
  {open&&<div data-no-swipe className="fixed inset-0 z-[180] flex items-end bg-slate-950/55 backdrop-blur-sm sm:items-center sm:justify-center sm:p-4" onClick={()=>!loading&&setOpen(false)}>
   <section onClick={e=>e.stopPropagation()} className="w-full max-h-[88svh] overflow-hidden rounded-t-[30px] bg-white shadow-2xl sm:max-w-xl sm:rounded-[30px]">
    <header className="flex items-center justify-between gap-3 bg-[var(--bitalis-primary)] p-4 text-white"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10"><Bot className="h-6 w-6"/></span><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[var(--bitalis-mint)]">Gemini · datos reales</p><h2 className="text-lg font-black">BITALIS IA</h2></div></div><button disabled={loading} onClick={()=>setOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 disabled:opacity-50" aria-label="Cerrar BITALIS IA"><X className="h-5 w-5"/></button></header>
    <div className="max-h-[58svh] overflow-y-auto p-3 sm:p-4">
     ...
     <div className="space-y-3">{messages.map((message,index)=><div key={index} className={message.role==='user'?'ml-10 rounded-3xl rounded-br-lg bg-[var(--bitalis-primary)] p-3 text-sm text-white':'mr-6 rounded-3xl rounded-bl-lg border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-slate-700'}><p className="whitespace-pre-wrap">{message.text}</p>{message.role==='assistant'&&message.tools?.length?<p className="mt-2 text-[9px] font-black uppercase tracking-[.1em] text-emerald-700">Consultó: {message.tools.join(' · ')}</p>:null}</div>)}</div>
     {loading&&<div className="mt-3 mr-20 flex items-center gap-2 rounded-3xl border border-violet-100 bg-violet-50 p-3 text-sm font-bold text-violet-800"><Loader2 className="h-4 w-4 animate-spin"/>Gemini está consultando BITALIS…</div>}
    ...
    <form onSubmit={submit} className="border-t border-slate-200 bg-white p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] sm:p-4"><div className="flex gap-2"><textarea value={input} onChange={e=>setInput(e.target.value.slice(0,1200))} rows={2} placeholder="Ej. ¿Qué clientes tienen mayor prioridad hoy?" className="min-h-14 flex-1 resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-[var(--bitalis-action)]"/><button disabled={loading||!input.trim()} className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[var(--bitalis-action)] text-white disabled:opacity-40" aria-label="Enviar a BITALIS IA">{loading?<Loader2 className="h-5 w-5 animate-spin"/>:<Send className="h-5 w-5"/>}</button></div><p className="mt-2 text-center text-[9px] font-bold text-slate-400">BITALIS IA no registra pagos ni modifica datos en esta etapa.</p></form>
   </section>
  </div>}
 </>;
}
