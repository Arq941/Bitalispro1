...
export default function RouteEvidenceDrawer(){
 const[open,setOpen]=useState(false),[loading,setLoading]=useState(false),[credits,setCredits]=useState<Credit[]>([]),[media,setMedia]=useState<Media[]>([]),[clientId,setClientId]=useState(''),[clientName,setClientName]=useState('Cliente'),[error,setError]=useState('');
 const token=()=>localStorage.getItem('bitalis_access_token');
 const resolveCurrent=async()=>{
  const auth=token(); if(!auth)return;
  try{
   const r=await fetch('/api/collections/portfolio',{headers:{Authorization:`Bearer ${auth}`},cache:'no-store'});const j=await r.json().catch(()=>({}));if(!r.ok)return;const list:Credit[]=Array.isArray(j?.data)?j.data:[];setCredits(list);
