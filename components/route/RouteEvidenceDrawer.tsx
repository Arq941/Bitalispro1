...
export default function RouteEvidenceDrawer({showTrigger=true}:{showTrigger?:boolean}){
 ...
 return <>
  {showTrigger&&<button type="button" onClick={()=>setOpen(true)} aria-label={`Ver fotos de ${clientName}`} title="Ver evidencias" className="fixed bottom-28 right-3 z-[75] flex h-14 w-14 items-center justify-center overflow-hidden rounded-full border-2 border-white bg-slate-950 text-white shadow-2xl shadow-black/40 active:scale-95 print:hidden sm:right-4">
    {facade?.storageKey?<EvidenceImage storageKey={facade.storageKey} alt="Fachada" className="h-full w-full !rounded-none"/>:<Camera className="h-6 w-6 text-emerald-300"/>}
  </button>}
