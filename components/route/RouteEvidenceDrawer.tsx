...
export default function RouteEvidenceDrawer(){
 ...
 return <>
  <button type="button" onClick={()=>setOpen(true)} aria-label={`Ver fotos de ${clientName}`} className="fixed bottom-20 right-4 z-[85] flex min-h-16 max-w-[190px] items-center gap-2 rounded-2xl border border-white/10 bg-slate-950/95 p-2 pr-3 text-left text-white shadow-2xl shadow-black/40 backdrop-blur print:hidden">
    <div className="h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-white/10 bg-slate-900"><EvidenceImage storageKey={facade?.storageKey} alt="Fachada" className="h-12 w-12"/></div>
    <div className="min-w-0"><div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-[.12em] text-[#FF6A00]"><Camera className="h-3.5 w-3.5"/>Fachada</div><p className="mt-0.5 truncate text-[11px] font-black">{clientName}</p><p className="text-[9px] text-slate-400">Toca para ver 3 fotos</p></div>
  </button>
